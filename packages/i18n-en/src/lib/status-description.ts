...
export enum HttpStatusDescription {
   ...
   * Switching protocols：The requester has asked the server to switch protocols and the server acknowledges that it will do so.
   */
  SWITCHING_PROTOCOLS = 'Switching protocols：The requester has asked the server to switch protocols and the server acknowledges that it will do so.',
  /**
   ...
   * Partial content：The server delivers only a portion of the content, as requested by the client via a range header.
   */
  PARTIAL_CONTENT = 'Partial content：The server delivers only a portion of the content, as requested by the client via a range header.',
  /**
   ...
   * Use proxy：HTTP 1.1. The resource is only available through a proxy and the address is provided in response.
   */
  USE_PROXY = 'Use proxy：HTTP 1.1. The resource is only available through a proxy and the address is provided in response.',
  /**
   * SWITCH_PROXY: 
   *
   * Empty
   ...
   * Not found: The resource could not be found. This is often used as a catch-all for all invalid URIs requested by the server.
   */
  NOT_FOUND = 'Not found: The resource could not be found. This is often used as a catch-all for all invalid URIs requested by the server.',
  /**
   ...
   *
   * Empty
   * Web Server Is Down (CloudFlare)：The original server refused the connection from CloudFlare.
   */
  WEB_SERVER_IS_DOWN_CLOUD_FLARE = 'Web Server Is Down (CloudFlare)：The original server refused the connection from CloudFlare.',
  /**
   ...
   *
   * Empty
   * SSL Handshake Failed (CloudFlare)：CloudFlare could not negotiate an SSL/TLS handshake with the origin server.
   */
  SSL_HANDSHAKE_FAILED_CLOUD_FLARE = 'SSL Handshake Failed (CloudFlare)：CloudFlare could not negotiate an SSL/TLS handshake with the origin server.',
  /**
...
export enum HttpStatusDescriptionByCode {
   ...
   * Switching protocols：The requester has asked the server to switch protocols and the server acknowledges that it will do so.
   */
  h101 = 'Switching protocols：The requester has asked the server to switch protocols and the server acknowledges that it will do so.',
  /**
   ...
   * Partial content：The server delivers only a portion of the content, as requested by the client via a range header.
   */
  h206 = 'Partial content：The server delivers only a portion of the content, as requested by the client via a range header.',
  /**
   * h207: RFC 4918 11.1
   * https://datatracker.ietf.org/doc/html/rfc4918#section-11.1
   * Multi-status：The message body that follows is an XML message and can contain a number of separate response codes, depending on how many sub-requests were made.
   ...
   * Use proxy：HTTP 1.1. The resource is only available through a proxy and the address is provided in response.
   */
  h305 = 'Use proxy：HTTP 1.1. The resource is only available through a proxy and the address is provided in response.',
  /**
   * h306: 
   *
   * Empty
   ...
   * Not found: The resource could not be found. This is often used as a catch-all for all invalid URIs requested by the server.
   */
  h404 = 'Not found: The resource could not be found. This is often used as a catch-all for all invalid URIs requested by the server.',
  /**
   * h405: RFC 7231 6.5.5
   * https://datatracker.ietf.org/doc/html/rfc7231#section-6.5.5
   * Method not allowed: The resource was requested using a method that is not allowed. For example, requesting a resource via a POST method when the resource only supports the GET method.
   ...
   * Web Server Is Down (CloudFlare)：The original server refused the connection from CloudFlare.
   */
  h521 = 'Web Server Is Down (CloudFlare)：The original server refused the connection from CloudFlare.',
  /**
   * h522: 
   *
   * Empty
   ...
   * SSL Handshake Failed (CloudFlare)：CloudFlare could not negotiate an SSL/TLS handshake with the origin server.
   */
  h525 = 'SSL Handshake Failed (CloudFlare)：CloudFlare could not negotiate an SSL/TLS handshake with the origin server.',
  /**
   * h526: 
   *
   * Empty
